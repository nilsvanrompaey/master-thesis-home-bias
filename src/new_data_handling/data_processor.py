...
def compute_domestic_investments(cpis, wb, major):
    ...
    total_inward_investments = cpis.get_data(counterparts=major, countries="WR").droplevel(0).rename_axis("Country")
...
def reallocate_offshore_investments(investments, major, offshore):
    ...
    investments = investments.get_data(counterparts=sample, countries=sample)
    ...
    for year in investments.columns:
        S = investments.get_data(counterparts=sample, countries=sample, period=(year,year)).unstack(level="Country")
...
def compute_portfolio_excess_returns(ds, fed, major, weights):
    index_returns  = compute_index_excess_returns(ds, fed, major)
    weights.index.name, weights.columns.name = weights.columns.name, weights.index.name
    portfolio_returns = weights.T @ index_returns
...
#     total_in_sample = self.get_data(counterparts=sample_codes, countries="World", periods=range(2001, 2005)).groupby("Counterpart Country").sum()
...
#     size_of_index_in_portfolio = (self + offshore_distribution + domestic_investments).get_data(counterparts=sample, countries=sample)
...
#     weights = weights.get_data(countries=DS.CODES.copy(), periods=range(start, end+1)).mean(axis=1)
